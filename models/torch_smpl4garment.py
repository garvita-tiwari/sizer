...
class TorchSMPL4Garment(nn.Module):
    ...
    def forward(self, theta, beta=None, trans=None, garment_d=None,
    ...
    def forward_poseshaped(self, theta, beta=None, trans=None, garment_class=None):
        if not self.cur_device:
            device = theta.device
            self.cur_device = torch.device(device.type, device.index)
